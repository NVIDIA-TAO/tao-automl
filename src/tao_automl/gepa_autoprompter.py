...
try:
    ...
    class EvaluationBatch:
        """Minimal fallback matching GEPA's evaluation-batch container."""
...
class TAOActionBatchRunner:
    ...
    def run_batch(self, candidate: Mapping[str, Any], items: Sequence[dict[str, Any]]):
        """Apply candidate overrides and execute the configured TAO batch."""
...
class RoutedTAOActionBatchRunner:
    ...
    def run_batch(self, candidate: Mapping[str, Any], items: Sequence[dict[str, Any]]):
        """Route items to specialized candidates and restore input order."""
...
class GEPAReflectionLM:
    ...
    def __call__(self, prompt: str | list[dict[str, Any]]) -> str:
        """Generate a reflection response for a text prompt or chat messages."""
...
class TAOGEPAAdapter:
    ...
    def validate_seed_candidate(self, candidate: Mapping[str, Any]) -> None:
        """Validate that every bounded configuration starts with an allowed value."""
        for component, choices in self.config_choices.items():
    ...
    def full_candidate(self, candidate: Mapping[str, Any]) -> dict[str, Any]:
        """Combine fixed settings with the candidate being evaluated."""
    ...
    def run_outputs(
        ...
        """Evaluate a candidate, using the cache when enabled."""
        batch = list(items)
    ...
    def evaluate(
        ...
        """Evaluate each item and return scores, outputs, and optional traces."""
        items = list(batch)
    ...
    def make_reflective_dataset(
        ...
        """Build leak-free reflection records for the selected components."""
        full_candidate = self.full_candidate(candidate)
...
class AutoPrompterResult:
    """Final validation-selected candidate and its evaluation evidence."""
    ...
    def to_dict(self) -> dict[str, Any]:
        """Serialize the result fields that are safe to persist as JSON."""
        return {
...
class GEPAutoPrompter:
    ...
    def optimize(
        ...
        *,
        ...
        """Run GEPA and select the best candidate using validation evidence."""
        try:
            import gepa
